...
switch = setmetatable({},{__call=function(a,b)_G.case=setmetatable({},{__call=function(a,...)return a[{...}]end,__index=function(a,c)local d=false;if c and type(c)=="table"then for e=1,#c do local f=c[e]if f and b and f==b then d=true;break end end end;if d then return setmetatable({},{__call=function(a,g)_G.default=setmetatable({},{__call=function(a,h)end})g()end})else return function()end end end})_G.default=setmetatable({},{__call=function(a,b)if b and type(b)=="function"then b()end end})return a[b]end,__index=function(a,f)return setmetatable({},{__call=function(a,...)end})end})deepcopy = function(a,b)b=b or{}local c=type(a)local d;if c=='table'then if b[a]then d=b[a]else d={}b[a]=d;for e,f in next,a,nil do d[deepcopy(e,b)]=deepcopy(f,b)end;setmetatable(d,deepcopy(getmetatable(a),b))end else d=a end;return d end --http://lua-users.org/wiki/CopyTable
